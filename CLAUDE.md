...
- `config/` - Symlink to `~/.config` containing user configuration files for various applications
- `scripts/` - Reusable Python scripts for common admin tasks
...
When performing admin tasks that could be reused, create a Python script in `scripts/`:
...
3. **Add dependencies to pyproject.toml** - If external packages are needed, add them to `pyproject.toml`
...
- Check if a similar script already exists in `scripts/`
